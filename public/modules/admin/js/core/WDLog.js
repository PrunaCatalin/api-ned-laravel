...
Log.add = function (message , typeOfMessage = "log",element = null, when  = "isReady") {
    const e = new Error();
    if (!e.stack) {
        try {
    ...
    const info = Log.getFileInfo(stack[1]);
    Log.parseMessage({
        type: typeOfMessage,
        message: message,
...
Log.parseMessage = function (message, element) {
    /*
